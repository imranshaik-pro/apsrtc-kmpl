...
def validate_calculation_result(result):
    ...
    calculation invariants. It does not perform business
    calculations itself.
    ...
    expected_kmpl = total_kms / hsd

    if not isclose(kmpl, expected_kmpl, rel_tol=1e-12):
        raise ValueError(
            "KMPL does not match Total Kms / HSD."
        )
    ...
    if slab not in (1, 2, 3, 4, 5):
        raise ValueError(
            "slab must be between 1 and 5."
        )
