...
def _format_vehicle_report_date(report_date):
    """
    Convert YYYY-MM-DD to the APSRTC vehicle report format DD/MM/YYYY.
    """
...
def _collect_vehicle_records(
    session,
    report_date,
    vehicle_depot,
):
    vehicle_report_date = _format_vehicle_report_date(
        report_date
    )
...
def _select_depot_region_records(
    records,
    depot,
):
    depot_records = [
        ...
        if record.get("depot") == depot
    ]

    if not depot_records:
        raise ValueError(
            f"No Region records found for depot {depot!r}."
        )
...
def build_daily_report(
    ...
    """
    Build one complete APSRTC Telugu daily report.
    ...
    Data flow:
        ...
        Both
            ↓
        Telugu report builder

    Existing calculation and reporting components are reused.
    This function does not implement business calculations itself.
    """
    ...
    if not vehicle_records:
        raise RuntimeError(
            "Vehicle report returned no valid vehicle records."
        )

    for_day_results = calculate_for_day(
        vehicle_records
    )

    up_to_day_results = calculate_up_to_day(
        vehicle_records
    )
    ...
    region_reporting_data = build_region_reporting_data(
        depot_region_records
    )
...
def run_daily_report(
    ...
    """
    Authenticate once and build one complete daily report.
    """
