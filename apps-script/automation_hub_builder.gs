...
function upgradeExistingApsrtcAutomationHub() {
  ...
  if(!formId) throw new Error('AUTOMATION_HUB_FORM_ID is missing.');
  ...
  // Safe in-place strategy: do NOT delete/rebuild branched items.
  // Reuse the existing landing-page questions and remove duplicate depot
  // questions from service sections by making them non-required and clearly
  // legacy. This avoids Google Forms' branching mutation restriction.
  const items=form.getItems();
  const depotItems=items.filter(i=>String(i.getTitle()||'').trim().toLowerCase().includes('depot'));
  let commonDepot=depotItems.find(i=>String(i.getTitle()||'').trim().toLowerCase()==='depot');
  ...
  form.getItems().forEach(i=>{
    if(i.getId()===commonDepot.getId()) return;
    const t=String(i.getTitle()||'').trim();
    if(/depot/i.test(t)){
      if(i.getType()===FormApp.ItemType.LIST) i.asListItem().setRequired(false);
      if(i.getType()===FormApp.ItemType.MULTIPLE_CHOICE) i.asMultipleChoiceItem().setRequired(false);
      i.setTitle('Legacy - '+t+' (leave blank)');
    }
    if(/^Tyres Change Position$/i.test(t) && i.getType()===FormApp.ItemType.CHECKBOX){
      i.asCheckboxItem().setChoiceValues(['FOS','FNS','ROSI','ROSO','RNSO','RNSI','Spare']);
    }
  });

  form.setTitle('APSRTC – OPERATIONS & KPI AUTOMATION HUB')
    .setDescription('ANDHRA PRADESH STATE ROAD TRANSPORT CORPORATION (APSRTC)\\nUnified Depot Operations, Vehicle Events, KMPL Reporting & KPI Automation Portal\\n\\n1. Select Depot  •  2. Select Required Service  •  3. Enter only the relevant details  •  4. Submit')
  ...
  console.log('AUTOMATION_HUB_SAFE_UPGRADE_COMPLETED');
  console.log('FORM_EDIT_URL: '+form.getEditUrl());
}
